...
import { commitOrder, submitOrder } from '@/api/order';
export default {
  name: 'PayIndex',
  data() {
    return {
  ...
  methods: {
    ...
    async checkOrder() {
      ...
      const goodsObj = this.mode === 'cart' ? { cartIds: this.cartIds } : buyData;
      const {
        data: { order, personal },
      } = await commitOrder(this.mode, goodsObj);
      this.order = order;
    ...
    async submitOrder() {
      ...
      const cartData = { cartIds: this.cartIds, remark: this.remark };
      const goodsObj = this.mode === 'cart' ? cartData : buyData;
      const res = await submitOrder(this.mode, goodsObj);
      if (res.status === 200) {
        this.$toast.success('支付成功');
        // 使用 push 保留历史，这样用户从订单页返回仍能回到来源页面
