...
export default {
  ...
  data() {
    return {
      ...
      total: 0,
      defaultImg, // 默认显示的图片
      showPannel: false, // 弹框显示
      mode: 'buyNow', // 当前点击的是购物车还是立即购买
      addCount: 2, // 加入购物车的数量
      cartTotal: 0, // 购物车现在商品总数
    };
  },
  // 在计算属性中获取路径参数
  // 响应式 + 自动更新
  ...
  methods: {
    ...
    async getComment() {
      const {
        data: { list, total },
      } = await getComment(this.goodsId, 3);
